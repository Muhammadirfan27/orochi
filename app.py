...
# --- 4. LOGIKA IDLE (TIDUR) ---
# Jika > 5 detik tidak ada interaksi dan bukan dalam mode bicara/berpikir, maka tidur
if time.time() - st.session_state.last_interaction > 5 and st.session_state.status == "diam":
    st.session_state.status = "tidur"

# --- 5. CSS BACKGROUND (PATH FIX) ---
# Pastikan path URL mengarah ke folder templates/
...
st.markdown(f"""
    ...
    /* iframe tetap ada tapi tidak terlihat agar lokasi tetap terdeteksi */
...
# Jika user chat, paksa bangun ke mode berpikir
...
# --- 7. PROSES AI (STATE MACHINE) ---
if st.session_state.status == "berfikir":
    time.sleep(1) # Jeda berpikir terlihat oleh user
